...
static void PSI_Grid2grid(Grid* grid, psi_grid* cgrid) {
	...
	if(PyDict_Check(grid->fields)) {
		...
		while (PyDict_Next(grid->fields, &pos, &key, &value)) {
			char* cstring = PyUnicode_AsUTF8(key);
...
static int Grid_init(Grid *self, PyObject *args, PyObject *kwds) {
	...
	if(strcmp(type, "cart") == 0 && 
			PyArg_ParseTupleAndKeywords(args, kwds, "O|((ddd)(ddd))(iii)O", kwlist, // for cart 
		...
		if(fields != NULL && fields != Py_None) {
		    ...
		    for (i = 0; i < len; i++) {
				...
				if(PyUnicode_Check(item)) {
					char* cstring = PyUnicode_AsUTF8(item);
					if(strcmp(cstring, "m") == 0) {;}
					else if(strcmp(cstring, "x") == 0) {
						npdims[3] = 3; 
						dim = 4;
...
/* see https://stackoverflow.com/q/28857522 and https://bugs.python.org/issue11587 */
...
/* see https://stackoverflow.com/a/43622454 */
